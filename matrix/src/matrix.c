...
Matrix* mul_matrix_element(Matrix* a, Matrix* b) {
    if (a->rows != b->rows || a->cols != b->cols) {
        printf("Matrices do not have the same dimensions function 'mul_matrix_element' has failed");
        return NULL;
    }
    Matrix* m = (Matrix*) malloc(sizeof(Matrix));
    m->rows = a->rows;
...
Matrix* add_matrices(Matrix* a, Matrix* b) {
    if (a->rows != b->rows || a->cols != b->cols) {
        printf("Matrices do not have the same dimensions function 'add_matrices' has failed");
        return NULL;
    }
    Matrix* m = (Matrix*) malloc(sizeof(Matrix));
    m->rows = a->rows;
...
Matrix* sub_matrices(Matrix* a, Matrix* b) {
    if (a->rows != b->rows || a->cols != b->cols) {
        printf("Matrices do not have the same dimensions function 'sub_matrices' has failed");
        return NULL;
    }
    Matrix* m = (Matrix*) malloc(sizeof(Matrix));
    m->rows = a->rows;
...
Matrix* square_diff(Matrix* a, Matrix* b){
    if (a->rows != b->rows || a->cols != b->cols) {
        printf("Matrices do not have the same dimensions function 'square_diff' has failed");
        return NULL;
    }
    Matrix* m;
    double diff[3];
    for (int i = 0; i < a->rows; i++) {
        for (int j = 0; j < a->cols; j++) {
            diff[i] = ((a->data[i][j] - b->data[i][j])*(a->data[i][j] - b->data[i][j]))/2;
        }
    }
